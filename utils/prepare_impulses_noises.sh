...
if [ $stage -le 1 ]; then
  ...
  $decode_cmd JOB=1:$num_db_jobs $log_dir/log/DBprocess.JOB.log \
    db=\(0 $db_string \)  \&\& \
    utils/rirs/prep_\$\{db\[JOB\]\}.sh --file-splitter "$log_dir/file_splitter.py 10 " --download $download_rirs --sampling-rate $sampling_rate $RIR_home ${output_dir}_non_normalized $log_dir ||exit 1;
fi
...
if [ $stage -le 2 ]; then
  ...
  for i in `ls $log_dir/*type*.rir.list`; do
    ...
    python utils/normalize_wavs.py --is-room-impulse-response true $i $i.normval || exit 1;
  ...
  for i in `ls $log_dir/*type*.noise.list`; do
    echo "Processing files in $i"
    python utils/normalize_wavs.py --is-room-impulse-response false $i $i.normval || exit 1;
    norm_coefficient=`cat $i.normval`
    echo "" > $i.normalized
    while read file_name; do
      if [ ! -z $file_name ]; then
