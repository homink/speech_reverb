...
type_num=1
python utils/get_air_file_patterns.py $AIR_home > $log_dir/air_file_pattern
files_done=0
...
while read file_pattern output_file_name; do
 # output_file_name=`echo ${DBname}_type${type_num}_${file_count}_$output_file_name| tr '[:upper:]' '[:lower:]'`
  ...
  echo "utils/read_rir.py --output-sampling-rate $sampling_rate air '${file_pattern}' ${output_dir}/${output_file_name} || exit 1;" >> $command_file
