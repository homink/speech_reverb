...
import argparse, shlex, glob, math, os, random, sys, warnings, copy, imp, ast, subprocess

data_lib = imp.load_source('dml', 'utils/data_dir_manipulation_lib.py')
...
def ProduceRevWav(dict, out_dir):
    keys = dict.keys()
    for key in keys:
        value = dict[key]
        ...
        data_lib.RunKaldiCommand("{0} {1} {2} {3}"
            .format(value_list[3],' '.join(value_list[4:-3]),value_list[1],out_dir+"/"+key+".wav"))
...
def GenerateReverberatedWavScp(wav_scp,  # a dictionary whose values are the Kaldi-IO strings of the speech recordings
    ...
    ProduceRevWav(corrupted_wav_scp, output_dir)
